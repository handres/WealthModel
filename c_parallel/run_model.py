...
def get_exp_worth(individual_worths, percentile=0.0):
	...
	popt, pcov = curve_fit(func, np.asarray(dat[0]), np.asarray(dat[1]), p0=(1, 500))
...
if __name__ == "__main__":
	...
	delta_omega = 0.01
	time_period = 100
	market_type = 0 # Deprecated variable - has no effect on program
	exponent_power = 1
	percentile = 0.0
	compress = True
	...
	print "Average Wealth: {0}".format(float(sum(individual_worths))/len(individual_worths))
	print "Median Wealth: {0}".format(sorted(individual_worths)[int(len(individual_worths)/2)])
